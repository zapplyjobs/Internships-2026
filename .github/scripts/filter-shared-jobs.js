...
/**
 * Filter Shared Jobs for Internships-2026
 *
 * Reads jobs from jobs-data-2026 shared data and filters for:
 * - Internships only (is_internship: true)
...
function filterInternshipJobs(jobs) {
  ...
  console.log(`🔍 Filtered ${filtered.length} internship jobs (${jobs.length - filtered.length} excluded)`);
  return filtered;
}
 ...
 */
function main() {
  console.log('🔍 Filter Shared Jobs for Internships-2026');
  ...
  const filteredJobs = filterInternshipJobs(sharedJobs);
  ...
  console.log(`   Output: ${filteredJobs.length} internship jobs`);
  console.log(`   Filtered out: ${sharedJobs.length - filteredJobs.length} jobs`);

  // Count by job type
  const internships = filteredJobs.filter(j => j.is_internship).length;
...
module.exports = { filterInternshipJobs, loadSharedJobs };
